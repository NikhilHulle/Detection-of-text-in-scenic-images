...
class TextDetection(object):
    ...
    def detect(self):
        ...
        contours, hierarchies = cv2.findContours(dilated, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            ...
            #temp_file = "images/text.jpg"
        numb = 0 
        for i, (contour, hierarchy) in enumerate(zip(contours, hierarchies[0])):
            if hierarchy[-1] != -1:
                continue
            numb+=1
            temp_file = "images/text" + str(numb)+".jpg"
            if self.use_tesseract:
                
                x, y, w, h = cv2.boundingRect(contour)
                if (y - self.MARGIN > 0) and (y + h + self.MARGIN < self.height) and (x - self.MARGIN > 0) and (x + w + self.MARGIN < self.width):
                    cv2.imwrite(temp_file, self.gray_img[y - self.MARGIN:y + h + self.MARGIN, x - self.MARGIN:x + w + self.MARGIN])
                else:
                    cv2.imwrite(temp_file, self.gray_img[y:y + h, x:x + w])
                ...
                if string != u'':
                    ...
                    img_to_rotate = cv2.imread(temp_file)
                    ...
                    (h, w) = img_to_rotate.shape[:2]
                    center = (w // 2, h // 2)
                    M = cv2.getRotationMatrix2D(center, -rotate_angle, 1.0)
                    rotated = cv2.warpAffine(img_to_rotate, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
                    cv2.imwrite("images/text" + str(numb)+"r.jpg", rotated)
                    string = image_to_string(rotated)
                    #print(len(string))
